...
describe("url module import syntax works", () => {
  describe("import { URL } from 'url';", () => {
    it("should parse a url hostname", () => {
      const test_url = url.URL("https://www.example.com");
      assert.equal(test_url.protocol, "https:");
      assert.equal(test_url.host, "www.example.com");
      assert.equal(test_url.hostname, "www.example.com");
    });
    it("toString method works", () => {
      const test_url = url.URL("/base", "https://www.example.com");
      assert.equal(test_url.toString(), "https://www.example.com/base");
    });
  });
  ...
  describe("import { URLSearchParams } from 'url';", () => {
    it("should have a parameter if it exists", () => {
      ...
      const searchParams = url.URLSearchParams(paramsString);
